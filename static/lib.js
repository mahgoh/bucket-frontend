      ...
      switch (this.options[key].type) {
        case 'boolean':
          this.filter[key] = params.hasOwnProperty(key)
            ? this.parseBoolean(params[key])
            : null;
          break;
        case 'number':
          this.filter[key] = params.hasOwnProperty(key)
            ? this.parseNumber(params[key])
            : null;
          break;
        case 'sort':
          this.filter[key] = params.hasOwnProperty(key)
...
const Regex = {
  ...
  preset(preset, min, max, flags = 'i') {
    ...
    return new RegExp(
      Regex.presets[preset].replace('MIN', min).replace('MAX', max),
      flags
    );
  },
...
const Template = {
  ...
  render(identifier, rules, parent) {
    ...
    rules.forEach((rule) => {
      if (rule.textContent) {
        clone.querySelector(`[data-${rule.key}]`).textContent =
          rule.textContent;
      }
      ...
      if (rule.attr) {
        for (let key in rule.attr) {
          clone
            .querySelector(`[data-${rule.key}]`)
            .setAttribute(key, rule.attr[key]);
        }
      }
